...
class PhiRhoCodebookConfig:
    ...
    axis: Optional[int] = None  # If None, global codebook; if int, per-axis codebook
    codebook_dtype: torch.dtype = torch.float32
    device: Optional[Union[str, torch.device]] = None
    lloyd_refine_steps: int = 0  # Optional 1D Lloyd steps after deterministic init

    def __post_init__(self):
        if self.num_codebook_entries < 2:
            raise ValueError("num_codebook_entries must be >= 2")
        if self.lloyd_refine_steps < 0:
            raise ValueError("lloyd_refine_steps must be >= 0")
